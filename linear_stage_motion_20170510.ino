 ...
 * 609-915-6213
 * 
 * Purpose: This code is intended to use the Arduino as a stepper motor controller for linear stages. Step rate ramping is done by the Arduino controller.
 *
 * To Do:
 ...
 *      - determine which global variables must be volitile, removing this qualifier from other global variables
 */
...
const unsigned int N_MOT = 3;                       // number of small motors controlled by arduino
...
const boolean SWITCH_ACTIVE = HIGH;                 // level for activated switch, ************************ may change later ************************
const unsigned char SWITCH_MODE = CHANGE;           // when the interrupt is triggered: LOW, CHANGE, RISING, FALLING
//const unsigned char SWITCH_MODE = M01           // when the interrupt is triggered: LOW, CHANGE, RISING, FALLING
//const unsigned long SWITCH_DELAY = 1000;            // delay time in ms before interrupt code is executed again
const unsigned long SWITCH_DELAY = 50000;            // delay time in ms before interrupt code is executed again

// pin constants
const int SWITCH_PIN[] = {2,3,20,21};               // pin numbers for position switch interrupts
const int QLIMIT_PIN[] = {34,35,36,37};             // pin numbers for determining which switch has been triggered
...
volatile unsigned long switch_countP[] = {0,0,0,0};         // counter for number of switch triggers detected for Protection
...
volatile unsigned long switch_time[] = {0,0,0,0};           // time of last executed switch interrupt
...
void setup(){
    wdt_disable(); // For safety's sake, wd timer is disabled at the start
    ...
    if(Serial.available() > 0){
        if(serial_time == 0)
            ...
            if(!get_command()){
                ...
                com_success();
                Serial.print("mot_state[0]:"); Serial.print(mot_state[0]); Serial.print("\n");
                Serial.print("mot_state[1]:"); Serial.print(mot_state[1]); Serial.print("\n");
                Serial.print("mot_state[2]:"); Serial.print(mot_state[2]); Serial.print("\n");
            }
        }
    }
...
void protection(int mot){
  boolean sw_state = digitalRead(SWITCH_PIN[mot]);
  sw_state = sw_state==SWITCH_ACTIVE; // correct polarity
  
  boolean qlimit = digitalRead(QLIMIT_PIN[mot]);
  qlimit = qlimit==SWITCH_ACTIVE; // qlimit [0:+ 1:-]
  //if( mot == 0 && sw_state ) {Serial.print(qlimit); Serial.print("\n");}

  

  // direction of each motor [false = cw(-), true = ccw(+)]
  if( sw_state && mot_state[mot] ) {
    if( direction[mot] != qlimit ) {
    //if( direction[mot] == qlimit ) {
      Serial.print("DEBUG STOP :"); Serial.print(mot); Serial.print("\n"); 

      if( switch_countP[mot]++ > 2 ){
        Serial.print("DEBUG switch_countP: "); Serial.print(switch_countP[mot]); Serial.print("\n");
        e_stop[mot] = true;
        reset_mot(mot);
      }
    }
...
void switch_trigger(int mot){
    //Serial.print("DEBUG swith_trigger: "); Serial.print(mot); Serial.print("\n");
    
    // check time delay
    boolean time_check = (millis()-switch_time[mot])>SWITCH_DELAY;
    //Serial.print(millis()-switch_time[mot]); Serial.print( " > " ); Serial.print(SWITCH_DELAY); Serial.print("\n");
        
    // only perform actions if motor is active and delay time has passed
    if(mot_state[mot]&time_check){

        // record time
        switch_time[mot] = millis();
        ...
        if(sw_state){
            // which limit switch is active?
            boolean qlimit = digitalRead(QLIMIT_PIN[mot]);
            qlimit = qlimit==SWITCH_ACTIVE; // correct polarity
            if(qlimit)
                qresp[3] = '-'; // if switch furthest from lenses is high then negative limit is reached
            else
                qresp[3] = '+'; // if switch nearest to lenses is high then positive limit is reached
        }
        else
            qresp[3] = 'L';
        response_telegram(qresp);

        // handle switch change to active
        if(sw_state){
...
boolean get_command(){
    ...
    if(k > 0){ // double check that there is stuff to read
        ...
        else if(cmd_str[cmd_ind] == 'R'){ // RESET (8/30/2016)
            // Enables the mcu's watchdog timer for a reset in 250ms
...
void steps_on(int mot){
    Serial.print("DEBUG point steps_on0: "); Serial.print(mot); Serial.print("\n");
    Serial.print("TOIE1: "); Serial.print(TOIE1); 
    Serial.print(", TIMSK1: "); Serial.print(TIMSK1); Serial.print("\n");
    switch(mot){
        case 0:
            Serial.print("DEBUG point A"); Serial.print(", TIMSK1: "); Serial.print(TIMSK1); Serial.print("\n");
            TIMSK1 |= (1<<TOIE1);
            Serial.print("DEBUG point B"); Serial.print(", TIMSK1: "); Serial.print(TIMSK1); Serial.print("\n");
            break;
        case 1:
            TIMSK3 |= (1<<TOIE3);
            break;
    ...
    Serial.print("DEBUG point steps_on1\n");
}
